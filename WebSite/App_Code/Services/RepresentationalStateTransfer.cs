...
namespace MyCompany.Services
{
    ...
    public class RepresentationalStateTransferBase : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        protected virtual void CombineScripts(HttpContext context, bool isSaaS, string scriptName, string culture, string version)
        {
            ...
            if (isSaaS)
            {
                ...
                sb.AppendFormat("var __targetFramework=\'4.0\';__tf=4.0;__cothost=\'appfactory\';__appInfo=\'AppOnTime|" +
    ...
    public abstract class OAuthHandler
    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                FormsAuthenticationTicket ticket = new FormsAuthenticationTicket(0, user.UserName, DateTime.Now, DateTime.Now.AddHours(12), false, String.Empty);
                                string encrypted = FormsAuthentication.Encrypt(ticket);
                                if (services.AuthenticateUser(user.UserName, ("token:" + encrypted), false) != null)
        ...
        {
            context.Response.Cookies.Set(new HttpCookie("COT_SaaS", GetHandlerName()));
        }
        ...
        public virtual MembershipUser SyncUser()
        {
            ...
            if ((user == null) && !((Config["Sync User"] == "false")))
            {
                // create user
            ...
            if ((user != null) && !((Config["Sync Roles"] == "false")))
            {
                // verify roles
                List<string> roleList = GetRoles(user);
        ...
        public virtual List<string> GetRoles(MembershipUser user)
        {
        ...
        protected virtual void HandleException(HttpContext context, Exception error)
        {
            RepresentationalStateTransfer rst = new RepresentationalStateTransfer();
            context.Server.ClearError();
            context.Response.TrySkipIisCustomErrors = true;
            context.Response.ContentType = "text/xml";
            context.Response.Clear();
            using (XmlWriter writer = rst.CreateXmlWriter(context.Response.OutputStream))
            	rst.RenderException(context, error, writer);
        }
                ...
                if (service == "facebook")
                	...
                	if (service == "google")
                    	...
                    	if (service == "windowslive")
                        	return new WindowsLiveOAuthHandler();
                        else
                        	if (service == "sharepoint")
                            	return new SharePointOAuthHandler();
                            else
                            	if (service == "identityserver")
                                	return new IdentityServerOAuthHandler();
            }
            return null;
        }
    ...
    public partial class FacebookOAuthHandlerBase : OAuthHandler
    {
        ...
        public override void RequestAuthorizationCode()
        {
            string authUrl = String.Format("https://www.facebook.com/{0}/dialog/oauth?response_type=code&client_id={1}&redire" +
                    "ct_uri={2}&scope={3}&state={4}", GetVersion(), Config.ClientId, Config.RedirectUri, Uri.EscapeDataString(Scope), HttpUtility.UrlEncode(GetState()));
        ...
        {
            JObject userObj = Query("me?fields=email", false);
    ...
    public partial class GoogleOAuthHandlerBase : OAuthHandler
    {
        ...
        public override void RequestAuthorizationCode()
        {
            ...
            string authUrl = String.Format("https://accounts.google.com/o/oauth2/v2/auth?response_type=code&client_id={0}&red" +
                    "irect_uri={1}&scope={2}&state={3}&access_type={4}&prompt=select_account", Config.ClientId, Config.RedirectUri, Uri.EscapeDataString(Scope), HttpUtility.UrlEncode(GetState()), accessType);
            HttpContext.Current.Response.Redirect(authUrl);
        }
        ...
        public override string GetUserName()
        {
            JObject userObj = Query("userinfo/v2/me", false);
            _userId = ((string)(userObj["id"]));
            return ((string)(userObj["email"]));
        }
        
        public override List<string> GetRoles(MembershipUser user)
        {
            List<string> roles = base.GetRoles(user);
            if (Config["Sync Roles"] == "true")
            {
                JObject result = Query(("admin/directory/v1/groups?userKey=" + _userId), true);
                if (result != null)
    ...
    public class SharePointOAuthHandlerBase : OAuthHandler
    {
        ...
        public virtual string ClientUri
        {
            get
            {
                if (String.IsNullOrEmpty(_clientUri))
                {
                    _clientUri = Config["Client Uri"];
                    if (_clientUri.StartsWith("https://"))
                    	_clientUri = _clientUri.Substring(8);
                }
                return _clientUri;
            }
        }
        ...
        public override void RequestAuthorizationCode()
        {
            ...
            if (String.IsNullOrEmpty(version))
            	version = "15";
            string authUrl = String.Format("https://{0}/_layouts/{1}/OAuthAuthorize.aspx?client_id={2}&scope={3}&response_typ" +
                    "e=code&redirect_uri={4}&state={5}", ClientUri, version, Config.ClientId, Uri.EscapeDataString(Scope), Config.RedirectUri, GetState());
            HttpContext.Current.Response.Redirect(authUrl);
        }
        ...
        protected override WebRequest GetAccessTokenRequest(string code, bool refresh)
        {
            ...
            {
                WebRequest getRealm = WebRequest.Create((("https://" + ClientUri) 
                                + "/_vti_bin/client.svc"));
            ...
            string body = String.Format("grant_type={0}&client_id={1}%40{2}&client_secret={3}&{4}={5}&redirect_uri={6}&res" +
                    "ource=00000003-0000-0ff1-ce00-000000000000%2F{7}%40{2}", grantType, Config.ClientId, _realm, HttpUtility.UrlEncode(Config.ClientSecret), codeName, code, Config.RedirectUri, ClientUri);
            byte[] bodyBytes = Encoding.UTF8.GetBytes(body);
            request.ContentType = "application/x-www-form-urlencoded";
            request.ContentLength = bodyBytes.Length;
            using (Stream stream = request.GetRequestStream())
        ...
        protected override WebRequest GetQueryRequest(string method, string token)
        {
            WebRequest request = WebRequest.Create(String.Format("https://{0}/_api/{1}", ClientUri, method));
        ...
        public override string GetUserName()
        {
            JObject userObj = Query("Web/CurrentUser", false);
            _userId = ((string)(userObj["Id"]));
            return ((string)(userObj["LoginName"])).Split('|').Last();
        }
        ...
        {
            context.Response.Redirect(((StartPage + "?_showNavigation=") 
                            + _showNavigation));
        }
    }
    ...
    public partial class WindowsLiveOAuthHandlerBase : OAuthHandler
    {
        ...
        public override void RequestAuthorizationCode()
        {
            string authUrl = String.Format("https://login.live.com/oauth20_authorize.srf?client_id={0}&scope={1}&response_typ" +
                    "e=code&redirect_uri={1}&state={2}", Config.ClientId, Uri.EscapeDataString(Scope), Config.RedirectUri, HttpUtility.UrlEncode(GetState()));
            HttpContext.Current.Response.Redirect(authUrl);
        }
    ...
    public partial class IdentityServerOAuthHandlerBase : OAuthHandler
    {
        ...
        public override void RequestAuthorizationCode()
        {
            string authUrl = String.Format("{0}/connect/authorize?response_type=code&client_id={1}&redirect_uri={2}&scope={3}" +
                    "&state={4}", ClientUri, Config.ClientId, Config.RedirectUri, Uri.EscapeDataString(Scope), HttpUtility.UrlEncode(GetState()));
            HttpContext.Current.Response.Redirect(authUrl);
        }
